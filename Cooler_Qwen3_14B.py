...
# Colab compatibility: install packages and mount Google Drive if in Colab
def is_colab():
    try:
        import google.colab
        return True
...
if is_colab():
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "transformers", "datasets", "torch", "tqdm", "peft", "accelerate"])
...
with_thinking_path = '/content/drive/MyDrive/Big_Data/with_thinking.jsonl'
without_thinking_path = '/content/drive/MyDrive/Big_Data/without_thinking.jsonl'

with open(with_thinking_path) as f1, open(without_thinking_path) as f2:
    data = []
    for line in f1:
        sample = json.loads(line)
        sample['has_thinking'] = True
        data.append(sample)
    for line in f2:
        sample = json.loads(line)
        sample['has_thinking'] = False
        data.append(sample)

print(os.path.exists('/content/drive/MyDrive/Big_Data/with_thinking.jsonl'))
print(os.path.exists('/content/drive/MyDrive/Big_Data/without_thinking.jsonl'))
...
def detect_timeframe(sample):
    ...
    if "datetime_str" in sample:
        datetime_str = sample["datetime_str"]
        ...
        if re.search(r'\d{2}:\d{2}', datetime_str):
            ...
            timeframe = "hourly"
        elif re.search(r'\d{4}-\d{2}-\d{2}', datetime_str):
